...
function showData(data,rdata,url,year) {
	...
	$.each(rdata.children, function(key, val) {
		items.push('<tr class="alt"><td>' + '<a href="/node/' + year + '/' + val.id + '">' + val.data.title + '</a></td><td align="right">R$ ' + val.data.valor_tabela + '</td><td align="right">R$ ' + val.data.valor_usuario + '</td><td align="right">' + val.data.porcentagem + '%</td></tr>');
	});
